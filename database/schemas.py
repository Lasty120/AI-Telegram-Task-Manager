...
class TaskActionSchema(BaseModel):
    ...
    title: Optional[str] = Field(None, description="Короткое название задачи, например 'Встреча с Тинькофф'")
    # В идеале пусть ИИ отдает сразу ISO строку или timestamp, но для начала сойдет и текст даты
    target_time: Optional[str] = Field(None, description="Когда напомнить? Формат YYYY-MM-DD HH:MM. Если время не указано, оставь null")
